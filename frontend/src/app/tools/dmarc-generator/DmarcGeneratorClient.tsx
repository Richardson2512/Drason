        ...
        <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="relative">
                    ...
                    <button
                        onClick={handleCopy}
                        className="absolute top-2 right-2 px-3 py-1.5 bg-gray-700 hover:bg-gray-600 text-white text-xs rounded-lg transition-colors"
                    >
                        {copied ? 'Copied!' : 'Copy'}
                    </button>
                </div>
                <p className="mt-3 text-xs text-gray-500">
                    ...
                    <code className="bg-gray-100 px-1.5 py-0.5 rounded font-mono">{dnsRecordName}</code> in your DNS
                    provider.
                </p>
            ...
            {warnings.length > 0 && (
                <div className="space-y-2">
                    {warnings.map((w) => (
                        <div key={w} className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <input
                    ...
                    className="w-full max-w-md px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm transition-all"
                />
                {domain.trim() && (
                    <p className="mt-2 text-xs text-gray-500">
                        DNS Record Name:{' '}
                        <code className="bg-gray-100 px-1.5 py-0.5 rounded font-mono">{dnsRecordName}</code>
                    </p>
                )}
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <select
                    ...
                    className="w-full max-w-md px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm transition-all bg-white"
                >
                    <option value="none">None &mdash; Monitor only (recommended for new domains)</option>
                    <option value="quarantine">Quarantine &mdash; Send to spam folder</option>
                    <option value="reject">Reject &mdash; Block entirely</option>
                </select>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <select
                    ...
                    className="w-full max-w-md px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm transition-all bg-white"
                >
                    <option value="same">Same as domain policy</option>
                    <option value="none">None &mdash; Monitor only</option>
                    <option value="quarantine">Quarantine &mdash; Send to spam folder</option>
                    <option value="reject">Reject &mdash; Block entirely</option>
                </select>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <div className="flex gap-2">
                    <input
                        ...
                        className="flex-1 px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm transition-all"
                    />
                    <button
                        onClick={addRuaEmail}
                        className="px-6 py-3 bg-purple-600 text-white rounded-xl hover:bg-purple-700 text-sm font-semibold transition-colors shrink-0"
                    >
                        Add
                    </button>
                </div>
                {ruaEmails.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                        {ruaEmails.map((email) => (
                            <span
                                key={email}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-50 text-purple-700 rounded-lg text-sm font-mono border border-purple-100"
                            >
                                mailto:{email}
                                <button
                ...
                {rufEmails.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-3">
                        {rufEmails.map((email) => (
                            <span
                                key={email}
                                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-sm font-mono border border-gray-200"
                            >
                                mailto:{email}
                                <button
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                        <input
                            ...
                            className="w-16 px-3 py-2 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm text-center transition-all"
                        />
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                ...
                <select
                    value={ri}
                    onChange={(e) => setRi(e.target.value as ReportInterval)}
                    className="w-full max-w-md px-4 py-3 rounded-xl border border-gray-200 focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none text-sm transition-all bg-white"
                >
                    <option value="86400">Every 24 hours (daily &mdash; default)</option>
