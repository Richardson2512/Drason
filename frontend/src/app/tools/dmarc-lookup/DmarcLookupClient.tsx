...
export default function DmarcLookupClient() {
 ...
 {result.policy && (
 <span className={`inline-flex items-center px-3 py-1 text-xs font-bold ${policyBadgeColor(result.policy)}`}>
 p={result.policy}
 </span>
 )}
 </div>
 ...
 <span className={`inline-flex items-center px-3 py-1 text-xs font-bold shrink-0 ${policyBadgeColor(result.policy)}`}>
 {result.policy === 'reject' ? 'Reject' : result.policy === 'quarantine' ? 'Quarantine' : 'None'}
 </span>
 )}
 {t.tag === 'sp' && result.subdomainPolicy && (
 <span className={`inline-flex items-center px-3 py-1 text-xs font-bold shrink-0 ${policyBadgeColor(result.subdomainPolicy)}`}>
