            ...
            <div className="mt-16 space-y-12">
                ...
                <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    ...
                    <div className="space-y-4 text-gray-600 text-sm leading-relaxed">
                        ...
                        <p>
                            Each mechanism can be prefixed with a qualifier: <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">+</code> (pass, default), <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">-</code> (fail), <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">~</code> (softfail), or <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">?</code> (neutral). The qualifier determines what happens when a sending server matches that mechanism.
                        </p>
                    </div>
                </section>
                ...
                <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 md:p-8">
                    ...
                    <div className="space-y-6 text-gray-600 text-sm leading-relaxed">
                        <div>
                            ...
                            <p>
                                Each <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">include</code>, <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">a</code>, <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">mx</code>, <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">ptr</code>, <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">exists</code>, and <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">redirect</code> mechanism triggers a DNS lookup. Included domains may also have their own includes, which count toward your total. If the total exceeds 10, the SPF check returns PermError and all emails fail authentication. Fix this by replacing includes with direct IP addresses or using SPF flattening services.
                            </p>
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-900 mb-2">2. Missing Include for a Sending Service</h3>
                            <p>
                                Every third-party service that sends email on your behalf (Google Workspace, Microsoft 365, Mailchimp, SendGrid, Smartlead, etc.) needs its own <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">include:</code> mechanism in your SPF record. If you add a new email provider but forget to update SPF, emails from that provider will fail the SPF check. Always check your provider&apos;s documentation for the correct include value.
                            </p>
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-900 mb-2">3. Wrong Mechanism Order</h3>
                            <p>
                                SPF mechanisms are evaluated left to right. While order does not change the final result for most records, placing the <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">all</code> mechanism anywhere other than the end will cause everything after it to be ignored. Always put <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">-all</code> or <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">~all</code> as the last mechanism in the record.
                            </p>
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-900 mb-2">4. Using +all or ?all</h3>
                            <p>
                                Setting the all mechanism to <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">+all</code> (pass) means any server in the world can send email as your domain. This defeats the entire purpose of SPF. Similarly, <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">?all</code> (neutral) provides no indication to receivers about unauthorized senders. Use <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">~all</code> (softfail) as a minimum and <code className="px-1.5 py-0.5 bg-gray-100 rounded text-xs font-mono">-all</code> (hard fail) for maximum protection.
                            </p>
                        </div>
                        <div>
