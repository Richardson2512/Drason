...
function mapSmartleadEventType(eventType: string): EventType {
    ...
    switch (normalized) {
        ...
        default:
            return EventType.EMAIL_SENT; // Default fallback
    }
}
...
export const triggerEvent = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.error('[MONITOR] Error processing event:', error);
...
export const handleSmartleadWebhook = async (req: Request, res: Response) => {
    ...
    console.log('[WEBHOOK] Smartlead payload:', JSON.stringify(payload).substring(0, 300));

    // Generate idempotency key for deduplication
    const idempotencyKey = generateWebhookIdempotencyKey(payload);
    console.log(`[WEBHOOK] Idempotency key: ${idempotencyKey.substring(0, 12)}...`);
    ...
    if (!eventType) {
        console.log('[WEBHOOK] No event_type in payload, acknowledging anyway');
    ...
    try {
        ...
        if (!mailbox) {
            console.log(`[WEBHOOK] Mailbox ${emailAccountId} not found in system`);
        ...
        const { isNew } = await eventService.storeEvent({
        ...
        switch (eventType.toUpperCase()) {
            ...
            case 'BOUNCE':
                ...
                console.log(`[WEBHOOK] Bounce recorded for mailbox ${mailbox.id}`);
                break;

            case 'EMAIL_SENT':
            case 'SENT':
                await monitoringService.recordSent(mailbox.id, campaignId || '');
                console.log(`[WEBHOOK] Send recorded for mailbox ${mailbox.id}`);
                break;
        ...
        console.error('[WEBHOOK] Error processing Smartlead webhook:', error);
...
export const getMailboxHealth = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.error('[MONITOR] Error getting mailbox health:', error);
