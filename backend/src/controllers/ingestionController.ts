...
export const ingestLead = async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(`[HEALTH GATE] Lead: ${email} | Classification: ${healthResult.classification} | Score: ${healthResult.score}`);
        ...
        if (healthResult.classification === 'red') {
            console.log(`[HEALTH GATE] BLOCKED lead ${createdLead.id}: ${healthResult.reasons.join(', ')}`);
...
export const ingestClayWebhook = async (req: Request, res: Response) => {
    ...
    try {
        // === LEAD HEALTH GATE ===
        const healthResult = await leadHealthService.classifyLeadHealth(email);
        console.log(`[HEALTH GATE CLAY] Lead: ${email} | Classification: ${healthResult.classification} | Score: ${healthResult.score}`);
        ...
        // RED leads are blocked - don't route them
        if (healthResult.classification === 'red') {
            console.log(`[HEALTH GATE CLAY] BLOCKED lead ${createdLead.id}: ${healthResult.reasons.join(', ')}`);
            await auditLogService.logAction({
                organizationId,
                entity: 'lead',
                entityId: createdLead.id,
