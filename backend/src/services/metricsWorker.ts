...
async function updateDomainHealth(
    ...
    // Determine target state based on aggregate metrics
    if (currentState === DomainState.HEALTHY) {
        if (domainMetrics.atRiskCount >= MONITORING_THRESHOLDS.DOMAIN_WARNING_THRESHOLD) {
            targetState = DomainState.PAUSED;
            reason = `${domainMetrics.atRiskCount} mailboxes at risk - exceeds threshold`;
        } else if (domainMetrics.averageRiskScore >= MONITORING_THRESHOLDS.RISK_SCORE_WARNING) {
            targetState = DomainState.WARNING;
            reason = `Domain average risk ${domainMetrics.averageRiskScore} exceeds warning threshold`;
        }
    } else if (currentState === DomainState.WARNING) {
        if (domainMetrics.atRiskCount >= MONITORING_THRESHOLDS.DOMAIN_WARNING_THRESHOLD) {
            targetState = DomainState.PAUSED;
            reason = `${domainMetrics.atRiskCount} mailboxes at risk - exceeds threshold`;
        } else if (domainMetrics.averageRiskScore < MONITORING_THRESHOLDS.RISK_SCORE_WARNING * 0.5) {
