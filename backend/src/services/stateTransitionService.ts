...
export function isValidTransition(
    ...
    if (!transitions) {
        console.error(`[STATE] Unknown entity type: ${entityType}`);
        return false;
    }

    const allowedTransitions = transitions[fromState as keyof typeof transitions];
    if (!allowedTransitions) {
        console.error(`[STATE] Unknown state ${fromState} for ${entityType}`);
        return false;
    }
...
export async function executeTransition(
    ...
    if (!isValidTransition(entityTypeName, fromState, toState)) {
        console.log(`[STATE] Invalid transition: ${entityTypeName} ${entityId} ${fromState} -> ${toState}`);
        return {
            success: false,
    ...
    if (toState === MailboxState.RECOVERING || toState === DomainState.RECOVERING) {
        ...
        if (cooldownInfo.isInCooldown) {
            console.log(`[STATE] Entity ${entityId} is in cooldown until ${cooldownInfo.cooldownUntil}`);
            return {
                success: false,
                previousState: fromState,
                newState: fromState,
    ...
    try {
        ...
        if (toState === MailboxState.PAUSED || toState === DomainState.PAUSED) {
            ...
            console.log(`[STATE] Set cooldown for ${entityId}: ${cooldownMs}ms (until ${cooldownUntil})`);
            ...
            console.log(`[STATE] Cleared cooldown for ${entityId}`);
        }
        ...
        console.log(`[STATE] Transition complete: ${entityTypeName} ${entityId} ${fromState} -> ${toState}`);
